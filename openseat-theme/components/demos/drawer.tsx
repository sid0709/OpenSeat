...
type Room = { id: string; title: string; owner: string; budget: number; status: "Open" | "Review" | "Awarded"; bids: number };
const ROOMS: Room[] = [
  { id: "brand", title: "Brand refresh", owner: PEOPLE[0].name, budget: 2400, status: "Open", bids: 6 },
  { id: "landing", title: "Landing page copy", owner: PEOPLE[1].name, budget: 1800, status: "Review", bids: 2 },
  { id: "motion", title: "Motion system", owner: PEOPLE[2].name, budget: 3200, status: "Open", bids: 4 },
  { id: "deck", title: "Pitch deck", owner: PEOPLE[3].name, budget: 950, status: "Awarded", bids: 11 },
];
...
export default function DrawerDemo() {
  ...
  const matches = ROOMS.filter((r) => statuses.includes(r.status) && r.budget >= budget[0] && r.budget <= budget[1]);

  return (
    <Examples>
      <Preview align="start" label="Sides" description="start and end slide from the sides (mirrored under RTL); top and bottom span the width.">
        <Row>
          {SIDES.map((s) => (
        ...
        {SIDES.map((s) => (
          <Drawer key={s} side={s} size={s === "top" || s === "bottom" ? 280 : "sm"} isOpen={side === s} onOpenChange={(o) => setSide(o ? s : null)} title={`side="${s}"`}>
            <Text color="secondary">Press Escape, click the scrim, or use the close button.</Text>
          </Drawer>
        ))}
      </Preview>

      <Preview align="start" label="Sizes" description="sm 360, md 480 (default), lg 720, or full — any number or CSS length works too.">
        <Row>
          {SIZES.map((s) => (
        ...
        {SIZES.map((s) => (
          <Drawer key={s} size={s} isOpen={size === s} onOpenChange={(o) => setSize(o ? s : null)} title={`size="${s}"`}>
            <Text color="secondary">Wider drawers suit tables and side-by-side comparisons.</Text>
          </Drawer>
        ))}
      </Preview>

      <Preview label="Record details" description="Click a row. The drawer keeps the table in view — tabs, metadata, activity, and header actions.">
        <Table columns={COLUMNS} rows={ROOMS} rowKey={(r) => r.id} onRowClick={(r) => { setRoom(r); setRoomTab("details"); }} variant="plain" />
        <Drawer
          isOpen={room !== null}
          ...
          headerActions={
            <HStack gap={1}>
              ...
              <MoreMenu label="Room actions" items={[{ label: "Duplicate" }, { label: "Archive" }, { type: "divider" }, { label: "Delete", variant: "destructive" }]} />
            </HStack>
          }
          footer={
            <HStack gap={2} hAlign="end">
              <Button label="Invite people" icon={<Icon icon={icons.users} />} onClick={() => setInvite(true)} />
        ...
        <Drawer size="sm" isOpen={invite} onOpenChange={setInvite} title="Invite people" subtitle={room?.title} footer={<Button label="Done" variant="primary" onClick={() => setInvite(false)} />}>
          <List>
            {PEOPLE.map((p) => (
              <ListItem key={p.name} label={p.name} description={p.role} startContent={<Avatar name={p.name} size="sm" tooltip={false} />} endContent={<Button label="Invite" size="sm" />} />
            ))}
          </List>
        </Drawer>
        <Caption>Invite people opens a second drawer on top — stacked drawers for a quick side task.</Caption>
      </Preview>

      <Preview align="start" label="Edit form" description="purpose=&quot;form&quot; keeps focus inside; the footer keeps Save in reach as the form scrolls.">
        <Row>
          <Button label="Edit room" icon={<Icon icon={icons.edit} />} onClick={() => setEdit(true)} />
          <Caption>
            {title} · {visibility}
          </Caption>
        </Row>
        ...
        >
          <FormLayout>
            <TextInput label="Title" isRequired value={title} onChange={setTitle} status={tried && !title.trim() ? { type: "error", message: "Give the room a title." } : undefined} />
            ...
            <Selector
              ...
              options={[
                { value: "sealed", label: "Sealed", description: "Bids stay hidden until the deadline." },
      ...
      <Preview align="start" label="Filters" description="From the start edge, with a live count on the Apply button.">
        <Row>
          <Button label="Filters" icon={<Icon icon={icons.filter} />} endContent={<Badge label={statuses.length} variant="info" />} onClick={() => setFilters(true)} />
          <Caption>
        ...
        <Drawer
          ...
          footer={
            <HStack gap={2} hAlign="between">
              ...
              />
              <Button label={`Show ${matches.length} rooms`} variant="primary" onClick={() => setFilters(false)} />
            </HStack>
          }
        >
          <Stack gap={5}>
            <CheckboxList label="Status" value={statuses} onChange={setStatuses}>
              {(["Open", "Review", "Awarded"] as const).map((s) => (
                <CheckboxListItem key={s} value={s} label={s} endContent={<Text color="secondary">{ROOMS.filter((r) => r.status === s).length}</Text>} />
              ))}
            </CheckboxList>
            <Slider label="Budget" value={budget} onChange={setBudget} min={0} max={BUDGET_MAX} step={BUDGET_STEP} valueDisplay="text" formatValue={usd} />
      ...
      <Preview align="start" label="Mobile navigation" description="A side nav in a start drawer — the menu button pattern on small screens.">
        <Row>
      ...
      <Preview align="start" label="Notifications" description="A narrow end drawer with a list and a header action.">
        <Row>
          <Button
            label="Notifications"
            icon={<Icon icon={icons.bell} />}
            endContent={NOTIFICATIONS.length - read.length ? <Badge label={NOTIFICATIONS.length - read.length} variant="error" /> : undefined}
        ...
        <Drawer
          ...
          title="Notifications"
          headerActions={<Button label="Mark all read" size="sm" variant="ghost" onClick={() => setRead(NOTIFICATIONS.map((n) => n.who))} />}
        >
          <List hasDividers>
            {NOTIFICATIONS.map((n) => (
              <ListItem
